...
public class DefaultScheduleProcessor extends BeanProcessor {
    ...
    public Boolean process(Object object) {
        ...
        try {
            ...
            if (match == null) {
                ...
                match = SbdMatchInternalApis.getMatchByStartTimeAndCompetitorIds(newMatch.getStartTime(), homeTeamId, awayTeamId);
                if (match != null) {
                    //if the match exists ,add the partner info to this match Partners List,this method can auto-relate partner infos
                    List<Partner> currentPartners = match.getPartners();
                    if (currentPartners == null) currentPartners = Lists.newArrayList();
                    currentPartners.add(newMatch.getPartners().get(0));
                    match.setPartners(currentPartners);
                    SbdMatchInternalApis.saveMatch(match);
                    return true;
                }
            }
            if (match == null && !newMatch.getStatus().equals(MatchStatus.MATCH_CANCELED)) {
                SbdMatchInternalApis.saveMatch(newMatch);
                return true;
            } else {
                if (newMatch.getStatus() == MatchStatus.MATCH_CANCELED) {
                    SbdMatchInternalApis.deleteMatch(match.getId());
                    LOG.warn("The match is cancelled and not created }", match.getId());
                    return false;
                }
                ...
                if (SbdMatchInternalApis.saveMatch(match) > 0) {
                    LOG.warn("The match:{} is updated", match);
                }
            }
            return true;
        } catch (Exception e) {
    ...
    private List<String> getUpdatePropertyByStatus(com.lesports.api.common.MatchStatus status) {
        ...
        if (status.equals(MatchStatus.MATCH_NOT_START)) {
            ...
            return properties;
        } else if (status.equals(MatchStatus.MATCH_END)) {
            properties.add("status");
