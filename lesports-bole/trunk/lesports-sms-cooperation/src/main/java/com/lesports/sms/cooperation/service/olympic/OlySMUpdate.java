...
/**
 * Created by zhonglin on 2016/7/29.
 */
@Service("olySMUpdate")
public class OlySMUpdate {
    ...
    public void createSMOlyXml() {
        ...
        try {
            ...
            while (true) {
                ...
                if (CollectionUtils.isEmpty(videos)) {
                    if(curPage!=1){
                        ...
                        BaiduXmlUtil.createAndUploadXmlFile(document,"smOly-"+time,timePath ,fileSize, fileList, Constants.fileuploadflag);
                    }
                    break;
                }

                for (Video video : videos) {
                    Element videoElement = BaiduXmlUtil.createVideoElement(video,Constants.OLY_SM_VIDEO_CH);
            ...
            //创建索引文件
            BaiduXmlUtil.addAndUploadIndexXmlFile(fileList, "v.api.le.com","smOlyIndex.xml",Constants.fileuploadflag);

        } catch (Exception e) {
            LOG.error("createSmOlyXml error", e);
        }

        LOG.info("createSmOlyXml end");
    }
