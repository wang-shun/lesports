...
public abstract class AbstractSearchService<T extends Indexable, ID extends Serializable>
	...
	@Resource(name = "configMongoFactory")
	private MongoFactory configMongoFactory;

	@Resource(name = "sbcMongoFactory")
	private MongoFactory sbcMongoFactory;

	@Resource(name = "sbdMongoFactory")
	private MongoFactory sbdMongoFactory;

	@Resource(name = "transcodeMongoFactory")
	private MongoFactory transcodeMongoFactory;
	...
	@Override
	public final Long createIndexFromMongo(IndexType indexType, String db, String table, IndexMongoParam param) {
		MongoFactory factory = null;
		String index = indexType.getIndex();
		if (index.startsWith(MetaData.QMT_CONFIG)) {
			factory = configMongoFactory;
		} else if (index.startsWith(MetaData.QMT_SBC)) {
			factory = sbcMongoFactory;
		} else if (index.startsWith(MetaData.QMT_SBD)) {
			factory = sbdMongoFactory;
		} else if (index.startsWith(MetaData.QMT_TRANSCODE)) {
			factory = transcodeMongoFactory;
		}
		return param.isRpc() ? createIndexRpc(factory, db, table, param)
				: createIndexMongodb(factory, db, table, param);
	}

	private Long createIndexRpc(MongoFactory mongoFactory, String db, String table, IndexMongoParam param) {
		...
		if (updateAtRange != null && updateAtRange.length > 0 && updateAtRange.length <= 2) {
			...
			if (updateAtRange.length == 2 && StringUtils.isNotEmpty(updateAtRange[1])) {
	...
	private Long createIndexMongodb(MongoFactory mongoFactory, String db, String table, IndexMongoParam param) {
		...
		public void run() {
			try {
				...
				for (T entity : entities) {
					...
					list.add(indexQuery);
					LOG.debug("Entity {} is ready to be indexed, ID = {}", indexClass.getSimpleName(), entity.getId());
				}
				if (commonSearchRepository.saveBulk(list)) {
					LOG.info("Entities {} are indexed from RPC, ID = {}", indexClass.getSimpleName(), ids);
				} else {
					LOG.error("Entities {} index failed from RPC, ID = {}", indexClass.getSimpleName(), ids);
				}
			} catch (Exception e) {
				LOG.error("{} Index from RPC error, ID = {}", indexClass.getSimpleName(), ids, e);
			}
		}
	}
	...
	class IndexWorkerMongodb implements Runnable {
		...
		public void run() {
			try {
				...
				if (commonSearchRepository.saveBulk(list)) {
					LOG.info("Entities {} are indexed from Mongo, number = {}", indexClass.getSimpleName(),
							entities.size());
				} else {
					LOG.error("Entities {} index failed from Mongo, ID = {}", indexClass.getSimpleName(), entities);
				}
			} catch (Exception e) {
				LOG.error("{} Index from Mongo error, ID = {}", indexClass.getSimpleName(), entities, e);
			}
		}
	}
}
