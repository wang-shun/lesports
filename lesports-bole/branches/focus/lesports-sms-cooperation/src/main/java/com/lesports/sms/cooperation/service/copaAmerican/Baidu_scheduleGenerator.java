 ...
 */
@Service("BaiduScheduleService")
public class Baidu_scheduleGenerator extends AbstractService {
    private static final Logger LOG = LoggerFactory.getLogger(Baidu_scheduleGenerator.class);

    public String getFilePath() {
        String filePath = "scheduleandresult_mobile_copaAmerican2016.xml";
        return filePath;
    }
    ...
    private Element getDisplayElement() {
        ...
        displayElement.addContent(createElement("url", matchesUrl+baidu_logo));
    ...
    private List<Element> getVsboxElements(List<Match> matchList) {
        ...
        for (Match currentMatch : matchList) {
            ...
            attributes.put("icon1", logoMaps.get(homeTeam.getId()));
            attributes.put("record1", "");
            attributes.put("ranks2", awayTeam.getName());
            attributes.put("icon2", logoMaps.get(awayTeam.getId()));
