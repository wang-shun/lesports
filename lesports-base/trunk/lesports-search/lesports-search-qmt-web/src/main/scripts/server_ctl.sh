...
function usage(){
  ...
  echo "                  Default value is com.lesports.user.api.web"
  echo "    -m: Optional. Entry class name without package. Default is Main."
  echo "    -s: Optional. Service Name. Default is lesports-user-api-web."
}
...
JAVA18_HOME="/letv/apps/jdk1.8.0_91"
PKG="com.lesports.search.qmt"
NAME="SearchApiServer"
# Default port.
CMD=""
SERVICE_NAME="lesports-search-qmt-web"
...
if [ -f "/letv/apps/pinpoint-agent/pinpoint-bootstrap-1.6.0-SNAPSHOT.jar" ]
then
PINPOINT_ID=$(date +%N)
JAVA_OPTS=$JAVA_OPTS" -javaagent:/letv/apps/pinpoint-agent/pinpoint-bootstrap-1.6.0-SNAPSHOT.jar -Dpinpoint.applicationName=search-qmt -Dpinpoint.agentId=${PINPOINT_ID}"
fi

function start() {
    ${JAVA18_HOME}/bin/java ${JAVA_OPTS} -cp ${PHOME}/classes:${PHOME}/lib/* ${PKG}.${NAME} ${PORT} >> ${STD_LOG} 2>&1 &
    echo $! > ${PID_FILE}
    pid=`cat ${PID_FILE}`
