 ...
 */
public class SoccerScoreSectionFormatter implements Function<String, Long> {
    @Nullable
    @Override
    public Long apply(String input) {
        ...
        if (StringUtils.isEmpty(sectionName)) return 0L;
        InternalQuery currentQuery = new InternalQuery();
        currentQuery.addCriteria(new InternalCriteria("name", "is", "^" + sectionName + "$"));
