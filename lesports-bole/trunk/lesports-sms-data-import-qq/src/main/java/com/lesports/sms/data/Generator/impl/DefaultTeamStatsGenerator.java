 ...
 */
@Service("defaultTeamStatsGenerator")
public class DefaultTeamStatsGenerator extends Generator {
    @Resource
    FileParser fileParser;
    private static Logger LOG = LoggerFactory.getLogger(DefaultTeamStatsGenerator.class);

    public boolean nextProcessor() {
        List<TransModel> schedules = getFileUrl(Constants.teamStats);
        if (CollectionUtils.isEmpty(schedules)) {
            return false;
        }
        for (TransModel transModel : schedules) {
            fileParser.nextProcessor("TEAM-STATS", transModel);
        }
        return true;
    }
    ...
    private List<String> getValidCodeByCsidAndPartner(Long csid) {
        ...
        for (TeamSeason currentTeamSeason : teamSeasons) {
            Team currentTeam = SbdsInternalApis.getTeamById(currentTeamSeason.getTid());
            if (currentTeam == null){
                continue;
            }
            if (StringUtils.isBlankOrNull(currentTeam.getQQId())) continue;
            ids.add(currentTeam.getQQId());
        }
        return ids;
    }
}
